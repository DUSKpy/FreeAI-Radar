...
| M13 Safari/Firefox 降级路径验证 | ✅ | `RADAR_FALLBACK=1` 注入同一 blur 链；照片+降级 1944 元素全过 AA |

**当前测试基线：** `274 passed` · `ruff check` 全绿 · `ruff format --check` 全绿
...
## M15 — 卡片透明化：把"看不到照片"当成一个可测的数

### 用户反馈

"我需要将这一个一个小卡片也变得透明，能够看到后方背景中的图片，
并且让这些卡片的边缘具有流动玻璃的效果。"

### 先量，再改

没有直接调 token，而是先写 `.work/_card-glass-probe.mjs`：同一张卡片，
在**合成纯白照片**和**合成纯黑照片**下各渲染一次，量卡片自身像素的亮度中位数。
不透明的卡片两次读数应该完全相同；差值就是透出来的量。

实测：**照片净透出 4.6%**（88% scrim × 卡片 38% 透过）。
用户说"看不到图片"是字面成立的。

### 三个机制

1. **浅色 scrim 88% → 12% + `--photo-floor` + `background-blend-mode: lighten`**
   —— 从"往上盖"改成"从下托"。深色**不采用**：它的失败方向相反，
   `darken` 会把夜景照片唯一还读得出来的高光压平。
2. **Guard plates** —— 裸文字（报告正文、区块标题、分页、页脚）各自拿到
   82% 的局部底板。全页 scrim 必须按最坏位置定价，局部 guard 只需对得起
   它自己背后那行字。padding 配等量负 margin，布局零变化。
3. **`.glass--card.glass--edge::before` 沿边缘游走的高光**（9s 一轮）
   + 加大 `feTurbulence` 的 `baseFrequency` 动画幅度。

净透出：浅色 **4.6% → 约 48%**，深色 **2.9% → 约 14%**。

深色这一半是**补做**的。第一版只把浅色 scrim 降到 12%，深色保持 82%
（理由是"镜像的 `darken` 会压平夜景照片的高光"）。那个理由是错的 ——
深色根本不需要 floor：裸文字已经有 guard 底板，scrim 只需让**玻璃上方**
够暗，而不必替整页最坏位置兜底。降到 60% 后实测（纯白照片、导航层 88%）
4.81:1，仍然过线。

停在 60% 而不是更低，是因为约束来自导航层上 11px 的 `--text-subtle`：
50% 给 4.60、55% 给 4.73、60% 给 4.81，而离线模型比渲染乐观约 3%，
断言需要 4.635 —— 50% 能过浏览器但过不了模型。

**深色的导航层反而变厚**（44% → 88%，照片模式下）。这个主题的 tier 填充比
页面**亮**，照片越亮越把它往上推，11px 浅灰小字撑不住；卡片装的是
`--text` / `--text-muted`，都够亮，所以只有卡片能让照片多透。

### 一个反直觉的回归，被纯色审计抓住

我一度把深色导航层从 44% 加厚到 70%（"卡片都变薄了，nav 也该跟上"）。
纯色审计立刻报 **36 个失败**：`.sidebar__label` 掉到 4.03:1。

原因：深色 tier 的填充比页面**亮**，加厚让它更亮，11px 浅灰小字反而输。
卡片装的是 `--text` / `--text-muted`，两者都够亮，所以**只有卡片能变薄**。
两个 tier 反向移动是对的，不是不一致。

### ⚠️ 两个审计工具自己的 bug（比本轮任何 CSS 都重要）

它们让此前所有"全通过"的结论都不可信：

1. **`scroll-behavior: smooth`**（样式表在 `prefers-reduced-motion:
   no-preference` 下设的，而 headless Chromium 正是 no-preference）
   —— `window.scrollTo()` 启动**动画滚动**，脚本 160–200ms 后读 rect 时
   页面还在滑，第一屏之后的**所有**裁剪都错位。
2. **元素标记每个滚动步从 0 重新编号、互相覆盖** —— `querySelector` 于是
   返回**另一个元素**。

症状很隐蔽：页脚报出 `rgb(204,214,227)`，**黑白合成照片下完全相同**。
真实背景不可能不随照片变 —— 顺着这个反常才挖出来。

诊断靠 `.work/_el-probe.mjs`（直接报告一个元素自身 + 祖先链的 computed
background 和 crop 众数色）：页脚实际是 `rgb(246,249,252)` 占 77%，
对 `--text-subtle` 是 6.06:1。**一量就知道是工具错，不是设计错。**

第三个相关问题：元素半出视口时 clip 越界**不报错**，静默返回塞得下的行。
修法是裁剪前 `scrollIntoView({block:'center',behavior:'instant'})` 并重读 rect。
...
| --- | --- |
| 裸文字扫描 | 40 → **0**（两主题） |
| 结构最坏（合成纯黑/纯白 × 2 主题） | 163 元素 **0 失败**，最差 5.02/4.5 |
| 逐像素审计 photo | **1944 元素全通过 AA**，最紧 5.10/4.5 |
| 逐像素审计 solid | **1944 元素全通过 AA**，最紧 4.74/4.5 |
| 边缘流动 | 运动组帧间变化 0.61 vs 静止组 0.05（12 倍），行程 128px |
| 单元 | 全绿 · ruff All checks passed · 38 files formatted |

### 测试模型重写

`test_photo_backdrop_contrast.py` 从"全局 scrim 保护裸文字"改成
"guard plate 保护 + floor 约束 + tier 保护自己的内容"。
新增 `TIER_TEXT_TOKENS`：**card tier 上没有 `--text-subtle`**，
而深色 card 填充的亮度**高于** `--text-subtle` 需要的上限，
所以那个配对在任何 tier 不透明度下都不可能达标 ——
检查一个不存在的配对会逼着卡片变回不透明。

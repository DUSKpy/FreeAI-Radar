...
`docs/screens/` 共 108 张：96 张是「8 页 × 6 档宽度 × 2 主题」的完整矩阵，
另 12 张是专项截图——线上部署与手机外观控件 8 张，
再加 4 张图片背景专项（`photo-directory-{1440,390}-{light,dark}.png`）：
...
**M15 起，卡片边缘还有一道会走的高光。** `.glass--card.glass--edge::before`
用双层 `mask`（`content-box` 与整框做 `exclude`）把绘制限制在 2px 的环上，
再让一条 `linear-gradient` 沿 `background-position` 从 0% 扫到 100%（9s 一轮）。
**只给卡片**：nav/panel 用的是强折射，在已经被位移的背景上再叠一层屏幕空间的
渐变会错位成闪烁 —— 这正是 `glass.css` 顶部警告的那件事。

同一轮把 `feTurbulence` 的 `baseFrequency` 动画幅度加大，
让边缘的位移真的"走"起来，而不只是同一个场在原地"呼吸"。

"流动"在静态截图里看不出来，所以用连续采样验证：
`.work/_rim-flow-verify.mjs` 对同一条卡片上边缘连拍 7 帧，
**运动组帧间平均变化 0.61，关闭动效组 0.05（12 倍）**，
高光带在 846px 宽的卡片上行程 128px。动效偏好（`prefers-reduced-motion`
或页内开关）下这条伪元素整体 `display: none` —— 一条不动的高光环
就只是第二条边框。
...
**这一版推翻了上一版的结论。** 上一版写的是"浅色主题下照片只剩 12% 的透出，
读起来是一层很淡的色晕，而不是一张照片"，并把它称为"刻意选择的代价"。
代价是真的，但**定价方式是错的**：

上一版用一层全页 88% 的 scrim 去保护所有"直接落在背景上"的文字。
全页 scrim 必须按**最坏位置**定价，而账单由照片的每一个像素支付 ——
包括卡片后面的像素。实测照片净透出只有 **4.6%**（88% scrim × 卡片 38% 透过），
所以"卡片不透明、看不到照片"是字面成立的，不是错觉。

现在改成**三个局部机制**，各自只为自己那部分负责：
...
只是现在由照片的暗部单独支付，而不是整张照片。

想换回完全原样的照片，把 `--photo-floor` 改成 `transparent` 是唯一旋钮，
但要接受卡片内深色文字的对比度保证同时失效
（`tests/test_photo_backdrop_contrast.py` 会立刻变红，这正是它存在的意义）。

**验证**：两种背景模式各 **1944 个元素**逐像素审计，全部通过 AA
（纯色最紧 4.74:1，图片最紧 5.10:1）；结构最坏情况（合成纯黑/纯白照片）
163 个元素 0 失败，最差 5.02:1。

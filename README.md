...
界面用**液态玻璃**材质，纯 CSS，没有 WebGL，也没有为效果引入框架。
...
> `feDisplacementMap`）、扩散（`blur()`）、高光。其中**折射层没有 Safari 和
> Firefox 实现**，GitHub 上所有网页版实现都卡在这里。所以本项目的做法是：
> **所有浏览器都拿到一套完整的毛玻璃设计**，折射只在对的引擎上叠加。
